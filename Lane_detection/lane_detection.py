...
def thresholding(img, s_thresh=(100, 255), sx_thresh=(15, 255)):
	...
	h_channel = hls[:,:,0]
	# Sobel x
	sobelx = cv2.Sobel(l_channel, cv2.CV_64F, 1, 1) # Sobel filter
	abs_sobelx = np.absolute(sobelx) # Absolute x derivative to accentuate lines away from horizontal
	scaled_sobel = np.uint8(255*abs_sobelx/np.max(abs_sobelx)) #scale the 
	
	# Threshold x gradient
	sxbinary = np.zeros_like(scaled_sobel)
	#whichever pixel value > sx_thresh will stay. Rest will be zero.
